 ...
 *  @param      baud            baud MHz
  ...
  * @date     2019/5/27 星期一
  */
uint32 SPI_Init(SPIn_e spin, SPI_PCSn_e pcs, SPI_CFG master,uint8_t mode, uint32 baud)
{
    uint8  br,pbr;
...
SPI_CLK_EXIT:
        fit_clk =  bus_clk *1000 /(Scaler[fit_br] * Prescaler[fit_pbr]);

        //需要算一下 CS 到 CLK 的时间（波特率的半周期）,满足条件： (1<<(CSSCK + 1 ))*( 2*PCSSCK+1) = fclk /(2*  baud)
        clk =  bus_clk*1000/fit_clk/2;
        min_diff = ~0;
        fit_cssck = 0x0F;
        fit_pcssck = 3;
...
/**
  ...
  * @date     2019/5/27 星期一
  */
void SPI_RadeWrite(SPIn_e spin, SPI_PCSn_e pcs, uint8 *modata, uint8 *midata, uint32 len)
{
    uint32 i = 0;
    do
    {
